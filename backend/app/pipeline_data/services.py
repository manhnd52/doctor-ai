...
from app.pipeline_data.utils import (
    serialize_value,
    pipeline_context,
...
def create_pipeline_run(
    ...
    """
    existing = (
        db.query(PipelineRun).filter(PipelineRun.message_id == message_id).first()
    )
    if existing:
...
def get_pipeline_run_by_id(
    db: Session, user: User, run_id: int
    ...
    stmt = (
        ...
        .where(PipelineRun.id == run_id, ChatSession.user_id == user.id)
    )
        ...
        with pipeline_context():
            ...
            async for chunk in facade.astream_by_node(context):
                ...
                if node_name == "question_classification":
                    ...
                    for transition_event in tracker.handle_transition(
