...
system_prompt = """
  ...
  <label_candidates>
  These labels are confirmed to exist in the schema
  ...
  These relationship types exist in the schema
  and may be used to connect nodes.
  ...
  Prefer labels listed in `<label_candidates>` when matching new nodes.
  If a label contains spaces, wrap it in backticks (`).

  4. Relationship Usage
  Use ONLY relationship types defined in the schema.
  Prefer relationships listed in `<relationship_candidates>` when connecting nodes.
